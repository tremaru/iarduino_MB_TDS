...
bool	iarduino_MB_TDS::reset(void){																						//	
			...
			return MB_diagnostic(0x0001,0xFF00)<0? false:true;																//	Выполняем команду диагностики, функция 0x0001 с очисткой журнала событий 0xFF00.
}																															//
																															//
...
int32_t	iarduino_MB_TDS::getErr(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			return MB_diagnostic(0x0002);																					//	Выполняем команду диагностики, номер функции диагностики 0x0002 = получить регистр диагностики. Функция возвращает данные результата выполнения функции диагностики, или -1 при неудаче.
}																															//
																															//
...
bool	iarduino_MB_TDS::setStopCalib(void){																				//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return false; }												//	Ошибка синтаксиса, модуль не инициализирован.
			if( !MB_writeAO(AO_CALC_SAVE, 0x2709) ){ return false; }														//	Записываем значение 0x2709 в регистр "Holding Register" AO[AO_CALC_SAVE]. Функция возвращает 1 при успехе, 0 при неудаче.
...
int8_t	iarduino_MB_TDS::getStability(void){																				//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			return MB_readDI(DI_STABLE_S);																					//	Возвращаем значение регистра "Discrete Inputs" DI[DI_STABLE_S]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
}																															//
																															//
...
float	iarduino_MB_TDS::getFluctuation(uint8_t type){																		//	type - тип границы STABLE_S/UNSTABLE_S.
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i= -1.0f;																									//
			if( type==STABLE_S   ){ i=(float)MB_readAO(AO_STABLE_S  ); }													//	Читаем регистр "Holding Register" AO[AO_STABLE_S  ]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( type==UNSTABLE_S ){ i=(float)MB_readAO(AO_UNSTABLE_S); }													//	Читаем регистр "Holding Register" AO[AO_UNSTABLE_S]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0.0f ){ i/=10.0f; } return i;																			//	Возврашаем границу флуктуаций проводимости в десятых долях %.
}																															//
																															//
...
bool	iarduino_MB_TDS::setFluctuation(uint8_t type, float limit){															//	type - тип границы STABLE_S/UNSTABLE_S, limit - процент флуктуации проводимости за пределами которого будет меняться флаг стабильности показаний проводимости.
			...
			if( !MB_writeAO(AO_CALC_SAVE, 0x2709) ){ return false; }														//	Записываем значение 0x2709 в регистр "Holding Register" AO[AO_CALC_SAVE]. Функция возвращает 1 при успехе, 0 при неудаче.
			if( type==STABLE_S   ){ i = MB_writeAO(AO_STABLE_S  , (uint16_t)(limit*10.0f) ); }								//	Записываем в регистр "Holding Register" AO[AO_STABLE_S  ] процент отклонения проводимости в десятых долях. Функция возвращает 1 при успехе, 0 при неудаче.
			if( type==UNSTABLE_S ){ i = MB_writeAO(AO_UNSTABLE_S, (uint16_t)(limit*10.0f) ); }								//	Записываем в регистр "Holding Register" AO[AO_UNSTABLE_S] процент отклонения проводимости в десятых долях. Функция возвращает 1 при успехе, 0 при неудаче.
...
int32_t	iarduino_MB_TDS::getFrequency(void){																				//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			return MB_readAO(AO_FREQUENCY);																					//	Читаем частоту из регистра "Holding Register" AO[AO_FREQUENCY]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
}																															//
																															//
...
bool	iarduino_MB_TDS::setFrequency(uint16_t f){																			//	f - частота от 50 до 5000Гц.
			...
			if( !MB_writeAO(AO_CALC_SAVE, 0x2709) ){ return false; }														//	Записываем значение 0x2709 в регистр "Holding Register" AO[AO_CALC_SAVE]. Функция возвращает 1 при успехе, 0 при неудаче.
			bool i = MB_writeAO(AO_FREQUENCY, f);																			//	Записываем в регистр "Holding Register" AO[AO_FREQUENCY] частоту в Гц. Функция возвращает 1 при успехе, 0 при неудаче.
			delay(50);																										//	Ждём 50 мс, пока модуль не сохранит новые данные в flash память.
			return i;																										//
}																															//
																															//
...
int32_t	iarduino_MB_TDS::getKnownTDS(uint8_t stage){																		//	stage - стадия 1 или 2.
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			int32_t i=-1;																									//
			if( stage==1 ){ i=MB_readAO(AO_KNOWN_TDS_1); }																	//	Читаем регистр "Holding Register" AO[AO_KNOWN_TDS_1]. Функция возвращает прочитанное значение (0...65'535), или -1 при неудаче.
			if( stage==2 ){ i=MB_readAO(AO_KNOWN_TDS_2); }																	//	Читаем регистр "Holding Register" AO[AO_KNOWN_TDS_2]. Функция возвращает прочитанное значение (0...65'535), или -1 при неудаче.
...
bool	iarduino_MB_TDS::setKnownTDS(uint8_t stage, uint16_t tds){															//	stage - стадия 1 или 2, tds - количество растворенных примесей от 50 до 10'000.
			...
			if( !MB_writeAO(AO_CALC_SAVE, 0x2709) ){ return false; }														//	Записываем значение 0x2709 в регистр "Holding Register" AO[AO_CALC_SAVE]. Функция возвращает 1 при успехе, 0 при неудаче.
			if( stage==1 ){ i = MB_writeAO(AO_KNOWN_TDS_1, tds ); }															//	Записываем в регистр "Holding Register" AO[AO_KNOWN_TDS_1] количество растворенных примесей в ppm. Функция возвращает 1 при успехе, 0 при неудаче.
			if( stage==2 ){ i = MB_writeAO(AO_KNOWN_TDS_2, tds ); }															//	Записываем в регистр "Holding Register" AO[AO_KNOWN_TDS_2] количество растворенных примесей в ppm. Функция возвращает 1 при успехе, 0 при неудаче.
			delay(50);																										//	Ждём 50 мс, пока модуль не сохранит новые данные в flash память.
			return i;																										//
}																															//
																															//
//		ФУНКЦИИ ДУБЛЁРЫ КАЛИБРОВОЧНЫХ ЖИДКОСТЕЙ (УКАЗЫВАЕМ EC в мСм/см, а не TDS в ppm):									//
float	iarduino_MB_TDS::getKnownEC(uint8_t stage			){ float Kp=getKp(); if(Kp<0.0f){return -1.0f;} return (float)getKnownTDS(stage) / Kp / 1000.0f;              }
bool	iarduino_MB_TDS::setKnownEC(uint8_t stage, float ec	){ float Kp=getKp(); if(Kp<0.0f){return false;} return        setKnownTDS(stage, (uint16_t)(Kp*ec*1000.0f) ); }
																															//
...
float	iarduino_MB_TDS::getKa(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			if( MB_requestFrom(HOLDING_REGISTERS, AO_Ka_L, 2) != 2 ){ return -1.0f; }										//	Читаем регистры "Holding Registers" начиная с регистра DI[AO_Ka_L], всего 2 регистра. Функция возвращает количество прочитанных значений, или 0 при неудаче.
...
float	iarduino_MB_TDS::getKb(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)MB_readAO(AO_Kb);																				//	Читаем модуль степени степенной функции из регистра "Holding Register" AO[AO_Kb]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0.0f ){ i/=1000.0f; } return i;																			//	Возврашаем модуль степени полученный в тысячных долях.
}																															//
																															//
...
bool	iarduino_MB_TDS::setKb(float Kb){																					//	Kb - модуль степени степенной функции от 0,001 до 65,535.
			...
			bool i = MB_writeAO(AO_Kb, (uint16_t)(Kb*1000.0f) );															//	Записываем в регистр "Holding Register" AO[AO_Kb] модуль степени в тысячных долях. Функция возвращает 1 при успехе, 0 при неудаче.
			delay(50);																										//	Ждём 50 мс, пока модуль не сохранит новые данные в flash память.
			return i;																										//
}																															//
																															//
...
float	iarduino_MB_TDS::getKt(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)MB_readAO(AO_Kt);																				//	Читаем температурный коэффициент из регистра "Holding Register" AO[AO_Kt]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0.0f ){ i/=10000.0f; } return i;																			//	Возврашаем температурный коэффициент полученный в десятитысячных долях.
}																															//
																															//
...
bool	iarduino_MB_TDS::setKt(float Kt){																					//	Kt - температурный коэффициент от 0,0001 до 6,5535.
			...
			return MB_writeAO(AO_Kt, (uint16_t)(Kt*10000.0) );																//	Записываем в регистр "Holding Register" AO[AO_Kt] температурный коэффициент в десятитысячных долях. Функция возвращает 1 при успехе, 0 при неудаче.
}																															//
																															//
...
float	iarduino_MB_TDS::getKp(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)MB_readAO(AO_Kp);																				//	Читаем коэффициент пересчёта из регистра "Holding Register" AO[AO_Kp]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0.0f ){ i/=100.0f; } return i;																			//	Возврашаем коэффициент пересчёта полученный в сотых долях.
}																															//
																															//
...
bool	iarduino_MB_TDS::setKp(float Kp){																					//	Kp - коэффициент пересчёта от 0,01 до 655,35.
			...
			return MB_writeAO(AO_Kp, (uint16_t)(Kp*100.0) );																//	Записываем в регистр "Holding Register" AO[AO_Kp] коэффициент пересчёта в сотых долях. Функция возвращает 1 при успехе, 0 при неудаче.
}																															//
																															//
...
bool	iarduino_MB_TDS::set_t(float t){																					//	t - температура от 0,00 до 100,00 °С.
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return false; }												//	Ошибка синтаксиса, модуль не инициализирован.
			if( t<0.0f || t>100.0f ){ return false; }																		//	Возвращаем флаг ошибки синтаксиса.
			return MB_writeAO(AO_t, (uint16_t)(t*100.0) );																	//	Записываем в регистр "Holding Register" AO[AO_t] температуру в сотых долях. Функция возвращает 1 при успехе, 0 при неудаче.
}																															//
																															//
...
float	iarduino_MB_TDS::get_T(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)MB_readAO(AO_T);																					//	Читаем опорную температуру из регистра "Holding Register" AO[AO_T]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0.0f ){ i/=100.0f; } return i;																			//	Возврашаем опорную температуру полученную в сотых долях.
}																															//
																															//
...
bool	iarduino_MB_TDS::set_T(float t){																					//	t - температура от 0,00 до 100,00 °С.
			...
			return MB_writeAO(AO_T, (uint16_t)(t*100.0) );																	//	Записываем в регистр "Holding Register" AO[AO_T] опорную температуру в сотых долях. Функция возвращает 1 при успехе, 0 при неудаче.
}																															//
																															//
...
float	iarduino_MB_TDS::getVsens(void){																					//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)MB_readAI(AI_VSEN);																				//	Читаем напряжение на входе датчика из регистра "Input Register" AI[AI_VSEN]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0.0f ){ i/=10000.0f; } return i;																			//	Возврашаем напряжение на входе датчика полученное в десятитысячных долях В.
}																															//
																															//
...
float	iarduino_MB_TDS::getVo(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)MB_readAI(AI_V0);																				//	Читаем напряжение виртуального ноля на входе ОУ из регистра "Input Register" AI[AI_V0]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0.0f ){ i/=10000.0f; } return i;																			//	Возврашаем напряжение виртуального ноля на входе ОУ полученное в десятитысячных долях В.
}																															//
																															//
...
float	iarduino_MB_TDS::getVout(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)MB_readAI(AI_VOUT);																				//	Читаем напряжение на выходе ОУ из регистра "Input Register" AI[AI_VOUT]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0.0f ){ i/=10000.0f; } return i;																			//	Возврашаем напряжение на выходе ОУ полученное в десятитысячных долях В.
}																															//
																															//
...
int32_t	iarduino_MB_TDS::getRo(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			int32_t i = MB_readAI(AI_Ro);																					//	Читаем общее измеренное сопротивление из регистра "Input Register" AI[AI_Ro]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
...
float	iarduino_MB_TDS::get_S(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)MB_readAI(AI_S);																					//	Читаем измеренную удельную электропроводность из регистра "Input Register" AI[AI_S]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0.0f ){ i/=1000.0f; } return i;																			//	Возврашаем измеренную удельную электропроводность в мСм/см.
}																															//
																															//
...
float	iarduino_MB_TDS::getEC(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)MB_readAI(AI_EC);																				//	Читаем приведённую удельную электропроводность из регистра "Input Register" AI[AI_EC]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0.0f ){ i/=1000.0f; } return i;																			//	Возврашаем приведённую удельную электропроводность в мСм/см.
}																															//
																															//
...
int32_t	iarduino_MB_TDS::getTDS(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			return MB_readAI(AI_TDS);																						//	Возвращаем количество растворённых твёрдых веществ из регистра "Input Register" AI[AI_TDS]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
}																															//
																															//
...
bool	iarduino_MB_TDS::print(uint8_t type){																				//	type - тип выводимых данных: MB_TDS_Ro/S/EC/TDS/5V/Vo/Vout/Vsen.
			...
			return MB_writeAO(AO_DISPLAY, type);																			//	Записываем в регистр "Holding Register" AO[AO_DISPLAY] тип выводимых данных. Функция возвращает 1 при успехе, 0 при неудаче.
}																															//
																															//
/**		ФУНКЦИИ ДУБЛЁРЫ ОБЪЕКТА objModbus ДЛЯ ЧТЕНИЯ/ЗАПИСИ С НЕСКОЛЬКИХ ПОПЫТОК	**/										//
																															//
int8_t		iarduino_MB_TDS::MB_readDO		(uint16_t reg							){ uint8_t cntRW=maxRW; while(cntRW){ int8_t   i=objModbus->coilRead            (valID, reg);        if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
int8_t		iarduino_MB_TDS::MB_readDI		(uint16_t reg							){ uint8_t cntRW=maxRW; while(cntRW){ int8_t   i=objModbus->discreteInputRead   (valID, reg);        if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
int32_t		iarduino_MB_TDS::MB_readAO		(uint16_t reg							){ uint8_t cntRW=maxRW; while(cntRW){ int32_t  i=objModbus->holdingRegisterRead (valID, reg);        if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
int32_t		iarduino_MB_TDS::MB_readAI		(uint16_t reg							){ uint8_t cntRW=maxRW; while(cntRW){ int32_t  i=objModbus->inputRegisterRead   (valID, reg);        if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
uint16_t	iarduino_MB_TDS::MB_requestFrom	(uint8_t  type, uint16_t reg, uint16_t n){ uint8_t cntRW=maxRW; while(cntRW){ uint16_t i=objModbus->requestFrom         (valID, type,reg,n); if( i> 0 ){ return i;    }else{ cntRW--; } } return  0;    }
uint8_t		iarduino_MB_TDS::MB_getInfo		(uint8_t  id							){ uint8_t cntRW=maxRW; while(cntRW){ uint8_t  i=objModbus->getInfo             (id);                if( i> 0 ){ return i;    }else{ cntRW--; } } return  0;    }
int32_t		iarduino_MB_TDS::MB_diagnostic	(uint16_t func, uint16_t data			){ uint8_t cntRW=maxRW; while(cntRW){ int32_t  i=objModbus->diagnostic          (valID, func, data); if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
bool		iarduino_MB_TDS::MB_changeID	(uint8_t  newID							){ uint8_t cntRW=maxRW; while(cntRW){        if( objModbus->changeID            (valID, newID)  ){ valID=newID;  return true; }else{ cntRW--; } } return false; }
bool		iarduino_MB_TDS::MB_writeDO		(uint16_t reg, bool val					){ uint8_t cntRW=maxRW; while(cntRW){        if( objModbus->coilWrite           (valID, reg, val)             ){ return true; }else{ cntRW--; } } return false; }
bool		iarduino_MB_TDS::MB_writeAO		(uint16_t reg, uint16_t val				){ uint8_t cntRW=maxRW; while(cntRW){        if( objModbus->holdingRegisterWrite(valID, reg, val)             ){ return true; }else{ cntRW--; } } return false; }
