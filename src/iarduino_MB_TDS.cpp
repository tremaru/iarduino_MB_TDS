...
#define AO_CALC_SAVE	6			//	║ AO_CALC_SAVE  ║   0   ║ Код разрешения записи калибровочных значений в Flash память. Сбрасывается автоматически. Должен быть установлен в значение (0x2709) при запуске калибровки и до записи калибровочных значений напрямую через регистры (без калибровки).
									//	╟───────────────╫───────╢
#define AO_Kt			7			//	║ AO_Kt         ║ 0.0200║ Температурный коэффициент жидкости в десятитысячных долях.
#define AO_Kp			8			//	║ AO_Kp         ║  0.50 ║ Коэффициент пересчёта EC (мкСм/см) в TDS (ppm) в сотых долях.
#define AO_t			9			//	║ AO_t          ║ 25.00 ║ Температура жидкости (текущая) в сотых долях °С. Требуется обязательно указывать t перед установкой битов калибровки и чтением показаний.
#define AO_T			10			//	║ AO_T          ║ 25.00 ║ Опорная температура в сотых долях °С (обычно T = 20 или 25). Температура к которой требуется привести значение S для получения EC и TDS.
									//	╟───────────────╫───────╢
#define AO_DISPLAY		11			//	║ AO_DISPLAY    ║   0   ║ Тип данных на дисплее: 0-Ro, 1-S, 2-EC, 3-TDS, 4-5V, 5-Vo, 6-Vout, 7-Vsen.
									//	╟───────────────╫───────╢
...
bool	iarduino_MB_TDS::begin(uint8_t id){																					//	id - адрес модуля на шине.
		...
		//	Проверяем устройство:																							//
			...
			if( objModbus->read()!=DEF_MODEL_TDS)	{ return false; }														//	Идентификатор устройства не совпадает с DEF_MODEL_TDS.
...
bool	iarduino_MB_TDS::setKa(float Ka){																					//	Ka - множитель степенной функции от 0,001 до 4'294'967,295.
			...
			return objModbus->endTransmission();																			//	Выполняем инициированную ранее запись. Функция возвращает 1 при успехе, 0 при неудаче.
}																															//
																															//
...
int32_t	iarduino_MB_TDS::getRo(void){																						//
			...
			if( i>=0 ){ i*=10; } return i;																					//	Возврашаем общее измеренное сопротивление полученное в даОм (1 дека Ом = 10 Ом).
																															...
																															//
//		ФУНКЦИЯ ПОЛУЧЕНИЯ ИЗМЕРЕННОЙ УДЕЛЬНОЙ ЭЛЕКТРОПРОВОДНОСТИ:															//	Возвращает измеренную удельную электропроводность жидкости в мкСм/см, или -1 при провале чтения.
int32_t	iarduino_MB_TDS::get_S(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			return objModbus->inputRegisterRead(valID, AI_S);																//	Возвращаем удельную электропроводность из регистра "Input Register" AI[AI_S]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
}																															//
																															//
//		ФУНКЦИЯ ПОЛУЧЕНИЯ ПРИВЕДЁННОЙ УДЕЛЬНОЙ ЭЛЕКТРОПРОВОДНОСТИ:															//	Возвращает приведённую удельную электропроводность жидкости в мкСм/см, или -1 при провале чтения.
int32_t	iarduino_MB_TDS::getEC(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			return objModbus->inputRegisterRead(valID, AI_EC);																//	Возвращаем удельную электропроводность из регистра "Input Register" AI[AI_EC]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
}																															//
																															//
